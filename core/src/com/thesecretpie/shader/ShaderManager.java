...
/**
 * @author Przemek Muller This class simplifies usage of shaders and framebuffers. It can load shaders from files, fix
 *         them for GLES specifics and reload them at runtime.
 * 
 */
public class ShaderManager {
	 ...
	 * @param am
	 *            - your app's AssetManager instance
	 */
	...
	public ShaderManager(String shaderDir, AssetManager am, boolean addProcessors) {
		...
		if (addProcessors
				&& (Gdx.app.getType() == ApplicationType.Desktop || Gdx.app.getType() == ApplicationType.Applet || Gdx.app
						.getType() == ApplicationType.WebGL)) {
	 ...
	 * 
	 * @param fbIdn
	 *            - this framebuffer's identifier
	 * @param format
	 *            - pixel format of this framebuffer
	 * @param fbWidth
	 *            - desired width
	 * @param fbHeight
	 *            - desired height
	 * @param hasDepth
	 *            - whether to attach depth buffer
	 */
	...
	private void createScreenQuad() {
		if (screenQuad != null)
			return;
		screenQuad = new Mesh(true, 4, 6, new VertexAttribute(Usage.Position, 3, "a_position"), new VertexAttribute(
				Usage.ColorUnpacked, 4, "a_color"), new VertexAttribute(Usage.TextureCoordinates, 2, "a_texCoords"));
		...
		screenQuad.setVertices(new float[] { vec0.x, vec0.y, 0, 1, 1, 1, 1, 0, 1, vec1.x, vec0.y, 0, 1, 1, 1, 1, 1, 1,
				vec1.x, vec1.y, 0, 1, 1, 1, 1, 1, 0, vec0.x, vec1.y, 0, 1, 1, 1, 1, 0, 0 });
	 ...
	 * 
	 * @param shadIdn
	 *            - a shader with this identifier must be loaded earlier
	 * @return this ShaderProgram for chaining
	 */
	public ShaderProgram begin(String shadIdn) {
		if (currentShader != null)
			throw new IllegalArgumentException("Previous shader '" + currentShaderIdn
	 ...
	 * @param clearColor
	 *            - clear color for this framebuffer
	 */
	 ...
	 * 
	 * @param fbIdn
	 *            - framebuffer identifier to render to
	 * @param clearColor
	 *            - clear color for this framebuffer
	 */
	 ...
	 * @param out
	 *            - a Mesh to render the framebuffer to
	 * @param textureUniformName
	 *            - the name of the texture2d uniform parameter in the fragment shader
	 * @param textureId
	 *            - which texture number should be used
	 */
	 ...
	 * @param height
	 *            - new screen height
	 * @param resizeFramebuffers
	 *            - whether all of the framebuffers should be recreated to match new screen size
	 */
	 ...
	 *            - shader identifier
	 * @param fh
	 *            - FileHandle to vertex shader source
	 * @param fh2
	 *            - FileHandle to fragment shader source
	 */
	...
	public void add(AssetManager am, String key, String baseVertPath, String baseFragPath) {
		...
		while (!am.isLoaded(vertPath) || !am.isLoaded(fragPath)) {
			am.update();
			// Gdx.app.log("ShaderManager", am.getProgress() + ", " + am.getLoadedAssets() + "/" +
			// am.getQueuedAssets());
		}
	...
	/**
	 * Reloads all shaders from disk. Useful for writing shaders: edit and save shader source, reload in game. Should be
	...
	/**
	 * Sets the uniform with the given name, automatically binding the texture to a number. Throws an
	 * IllegalArgumentException in case it is not called in between a {@link #begin()}/{@link #end()} block.
	 * 
	 * @param name
	 *            the name of the uniform
	 */
	...
	/*
	 * public void setVertexAttribute(java.lang.String name, int size, int type, boolean normalize, int stride,
	 * java.nio.Buffer buffer) { if (currentShader != null) { currentShader.setVertexAttribute(name, size, type,
	 * normalize, stride, buffer); } else throw new
