...
public class Player extends AnimatedImage {
	...
	// Actor
	this.setSize(Const.PLAYER_SIZE * GVars.BOX_TO_WORLD, Const.PLAYER_SIZE
		* GVars.BOX_TO_WORLD);
	...
	body.applyLinearImpulse(
		impulse,
		body.getWorldCenter().add(0f, Const.BLOCK_HALF)
			.rotateRad(body.getAngle()), true);
	...
	this.setPosition((body.getPosition().x - Const.PLAYER_HALF)
		* GVars.BOX_TO_WORLD, (body.getPosition().y - Const.PLAYER_SIZE
		/ animFix)
		* GVars.BOX_TO_WORLD);

	if (GameObject.state != GameObject.ACTIVE)
	    return;
	Tween.to(this, ActorAccessor.ROTATE, 0.2f).target(angle + 270f)
